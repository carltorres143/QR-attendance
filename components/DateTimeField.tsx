  ...
  return (
    <View style={styles.group}>
      ...
      >
        ...
        <Ionicons name={mode === 'date' ? 'calendar-outline' : 'time-outline'} size={21} color={COLORS.primary} />
      </Pressable>
          ...
          <View accessibilityViewIsModal style={styles.modal}>
            <View style={styles.modalHeader}>
              ...
              <Pressable accessibilityRole="button" accessibilityLabel="Close picker" hitSlop={12} onPress={close} style={styles.close}>
                <Ionicons name="close" size={24} color={COLORS.primaryStrong} />
              </Pressable>
            </View>
            <DateTimePicker
              display={Platform.OS === 'ios' ? (mode === 'date' ? 'inline' : 'spinner') : mode === 'date' ? 'calendar' : 'clock'}
...
const styles = StyleSheet.create({
  ...
  modal: { backgroundColor: COLORS.card, borderRadius: 24, padding: 18, borderWidth: 2, borderColor: COLORS.primary, gap: 12 },
  modalHeader: { flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between' },
  modalTitle: { color: COLORS.primaryStrong, fontSize: 20, fontWeight: '800' },
